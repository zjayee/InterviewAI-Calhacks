...
class Interviewer:
  def __init__(self, audio, context=[]):
  ...
  def get_text_from_audio(self):
    input_text = self.gorq_client.audio.transcriptions.create(
      file=("sample_audio.m4a", self.audio),
  ...
  def get_response_from_gpt(self, input_text):
    response = self.open_ai_client.chat.completions.create(
        model="gpt-3.5-turbo",
        messages= self.context + [{"role": "user", "content":input_text}],
        n = 1,
        seed=0
    )
